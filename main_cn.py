...
def parse_task(text: str) -> Dict[str, Any]:
    ...
    if not name:
        raise ParseError("未能提取任务名称，请尝试更明确的描述。")
...
def validate_task(parsed: Dict[str, Any]) -> None:
    ...
    if due and due < date.today():
        raise ValidationError("截止日期已过期，请提供未来的日期。")
...
def execute_task(task: Task) -> str:
    ...
    if task.due_date:
        return f"任务已安排：{task.name}，截止日期 {task.due_date}，优先级 {task.priority}."
    return f"任务已安排：{task.name}，优先级 {task.priority}."
...
class TaskManager:
    ...
    def add_subtask(self, task_id: str, subtask: str) -> Task:
        if not subtask or not subtask.strip():
            raise ValidationError("子任务内容不能为空。")
        task = self.get_task(task_id)
        if not task:
            raise ValidationError("未找到指定任务。")
...
def list_tasks(tasks: List[Task]) -> str:
    ...
    lines = []
    for t in tasks:
        due = t.due_date or "未指定"
        header = (
            f"- {t.name} | 截止: {due} | 优先级: {t.priority} | 状态: {t.status}"
            f" | 描述: {t.description}"
        )
        lines.append(header)
        if t.subtasks:
...
def list_today_items(items: List[Dict[str, Any]], manager: TaskManager) -> str:
    ...
    for item in items:
        ...
        lines.append(f"- {task_name} | 子任务 {index}: {subtask}")
    return "\n".join(lines)


def print_help() -> None:
    print("可输入自然语言描述任务，例如：安排一个关于市场营销的会议，下周一，高优先。")
    print(
        "命令：list 查看任务；today 查看今天子任务；todayadd <task_id> <index> 添加今日子任务；"
        "todayrm <task_id> <index> 取消今日子任务；todaypick <关键词> 按关键词摘取今日子任务；"
        "delete <task_id> 删除任务；"
        "subadd <task_id> <子任务> 添加子任务；subrm <task_id> <index> 删除子任务；"
...
def main() -> None:
    manager = TaskManager()
    print("任务管理系统已启动。输入任务描述，或输入 help 查看帮助，list 查看已有任务，quit 退出。")
    if manager.tasks:
        print(f"已加载 {len(manager.tasks)} 条任务。")
    ...
    for line in sys.stdin:
        ...
        if user_input.lower().startswith("todayadd "):
            ...
            if len(parts) < 3:
                print("错误：用法 todayadd <task_id> <index>")
                continue
        ...
        if user_input.lower().startswith("todayrm "):
            parts = user_input.split(" ", 2)
            if len(parts) < 3:
                print("错误：用法 todayrm <task_id> <index>")
                continue
            task_id = parts[1].strip()
            try:
                index = int(parts[2].strip()) - 1
        ...
        if user_input.lower().startswith("subadd "):
            parts = user_input.split(" ", 2)
            if len(parts) < 3:
                print("错误：用法 subadd <task_id> <子任务>")
                continue
        ...
        if user_input.lower().startswith("subrm "):
            parts = user_input.split(" ", 2)
            if len(parts) < 3:
                print("错误：用法 subrm <task_id> <index>")
                continue
            task_id = parts[1].strip()
            try:
                index = int(parts[2].strip()) - 1
